...
export default async function ConfiguratorSettingsPage({ searchParams }: { searchParams: Promise<{ status?: string; version?: string; message?: string }> }) {
  ...
  return (
    <main className="mx-auto max-w-7xl px-6 py-10">
      <Link href="/admin" className="text-sm font-semibold text-emerald-800 hover:underline">← Zurück zum Dashboard</Link>
      <div className="mt-3 mb-8"><p className="text-sm font-semibold text-emerald-800">Einstellungen</p><h1 className="mt-1 text-3xl font-semibold text-slate-950">Konfiguratoren</h1><p className="mt-2 max-w-3xl text-slate-600">Zentrale Preis-, Wirtschafts- und Modellannahmen für Rechner, Konfiguratoren und Projektanalysen.</p></div>
