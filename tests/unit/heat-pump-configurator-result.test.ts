...
import { configuratorReducer, createInitialConfiguratorState } from "@/lib/configurator/state";
  ...
  describe("heat pump configurator result", () => {
    ...
    expect(buildHeatPumpConfiguratorResult(state)).toBeNull();
    });
  
    it("reuses the existing heat pump calculator", () => {
    const result = buildHeatPumpConfiguratorResult(createCompleteHeatPumpState());
    ...
    expect(result?.technicalReviewRecommended).toBe(false);
    });
    ...
    const result = buildHeatPumpConfiguratorResult(state);
      ...
      if (!result) {
      throw new Error("Expected heat pump result.");
      }
    ...
    expect(state.results.heatPump).toBeDefined();
